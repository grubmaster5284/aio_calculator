...
class CalculatorEngine {
  ...
  double? unary(String op, double x, {required bool isDegrees}) {
    switch (op) {
      case 'sin':
        return Math.sin(_toRadiansIfNeeded(x, isDegrees));
      case 'cos':
        return Math.cos(_toRadiansIfNeeded(x, isDegrees));
      case 'tan':
        return Math.tan(_toRadiansIfNeeded(x, isDegrees));
      case 'asin':
        final v = Math.asin(x);
  ...
  double _toRadiansIfNeeded(double degreesValue, bool isDegrees) =>
      isDegrees ? degreesValue * Math.pi / 180.0 : degreesValue;
  double _toDegrees(double radiansValue) => radiansValue * 180.0 / Math.pi;
